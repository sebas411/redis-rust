...
impl ClientHandler {
    ...
    async fn execute_command(&mut self, command: &str, args: Vec<RedisValue>) -> Result<Vec<u8>> {
        ...
        let response = match command {
                ...
                if args.len() < 2 {
                    ...
                    match acl_command.as_str() {
                        ...
                        "GETUSER" => {
                            ...
                            response = self.users.read().await.get(&user_req).unwrap().get_info().encode();
                        },
                        "SETUSER" => {
                            let user_req = args[2].get_string()?;
                            let password = args[3].get_string()?.strip_prefix('>').unwrap().to_string();
            ...
            "SETBIT" => {
                if args.len() != 4 {
                    RedisValue::Error("Err wrong number of arguments for 'SETBIT' command".to_string()).encode()
                } else {
                    let key = args[1].clone().get_string()?;
